...
const char* set_result_json(id object) {
    if (!object) return set_result_string(nil);
...
const char* uvclib_get_devices_json() {
    if (!g_uvcDevices) uvclib_refresh_devices();
...
const char* uvclib_get_controls_json() {
    if (!g_currentDevice) return set_result_string(@"[]");
...
const char* uvclib_get_value(const char* control_name) {
    if (!g_currentDevice) return set_result_string(@"{\"error\": \"No device selected\"}");
...
const char* uvclib_set_value(const char* control_name, const char* value_str) {
    ...
    if (control) {
        if ([control setCurrentValueFromCString:value_str flags:0]) {
            ...
            return set_result_string([NSString stringWithFormat:@"{\"error\": \"Invalid value format '%s' for %@\"}", value_str, nameStr]);
        }
    }
