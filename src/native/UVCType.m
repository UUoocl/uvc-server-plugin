...
{
    static NSUInteger byteSizes[] = {0, 1, 1, 1, 1, 2, 2, 2, 4, 4, 4, 8, 8, 8};
    if (componentType < kUVCTypeComponentTypeMax)
        return byteSizes[componentType];
    return 0;
}

//

const char *__UVCTypeComponentTypeString(UVCTypeComponentType componentType)
{
    static const char *typeStrings[] = {"<invalid>", "B",  "S1", "U1", "M1", "S2", "U2",
                                        "M2",        "S4", "U4", "M4", "S8", "U8", "M8"};
    if (componentType < kUVCTypeComponentTypeMax)
        return typeStrings[componentType];
    return 0;
}

//

const char *__UVCTypeComponentVerboseTypeString(UVCTypeComponentType componentType)
{
    static const char *verboseTypeStrings[] = {"<invalid>",
                                               "boolean",
    ...
    if (componentType < kUVCTypeComponentTypeMax)
        return verboseTypeStrings[componentType];
    return 0;
}

//

UVCTypeComponentType __UVCTypeComponentTypeFromString(const char *typeDefString, NSUInteger *nChar)
{
    ...
    if (outType != kUVCTypeComponentTypeInvalid)
        *nChar = charConsumed;
    return outType;
}
...
BOOL __UVCTypeComponentTypeScanf(const char *cString, UVCTypeComponentType theType, void *theValue,
                                 UVCTypeScanFlags flags, void *theMinimum, void *theMaximum, void *theStepSize,
                                 void *theDefaultValue, NSUInteger *nChar)
{
    BOOL rc = NO;
    ...
    if (strncasecmp(cString, "default", 7) == 0) {
        if ((flags & kUVCTypeScanFlagShowInfo))
            fprintf(stderr, "INFO: Default value requested\n");
        if (!theDefaultValue) {
            if ((flags & kUVCTypeScanFlagShowWarnings))
                fprintf(stderr, "WARNING: Default value requested, none provided by control\n");
            return NO;
        }
        switch (theType) {
        ...
        *nChar = strlen("default");
        ...
        if ((flags & kUVCTypeScanFlagShowInfo))
            fprintf(stderr, "INFO: Minimum value requested\n");
        if (!theMinimum) {
            if ((flags & kUVCTypeScanFlagShowWarnings))
                fprintf(stderr, "WARNING: Minimum value requested, none provided by control\n");
            return NO;
        }
        switch (theType) {
            case kUVCTypeComponentTypeSInt8: {
        ...
        *nChar = strlen("minimum");
        ...
        if ((flags & kUVCTypeScanFlagShowInfo))
            fprintf(stderr, "INFO: Maximum value requested\n");
        if (!theMaximum) {
            if ((flags & kUVCTypeScanFlagShowWarnings))
                fprintf(stderr, "WARNING: Maximum value requested, none provided by control\n");
            return NO;
        }
        switch (theType) {
            case kUVCTypeComponentTypeSInt8: {
    ...
    if (theType == kUVCTypeComponentTypeBoolean) {
        ...
        if ((flags & kUVCTypeScanFlagShowWarnings))
            fprintf(stderr, "WARNING: Invalid boolean value at '%s'\n", cString);
    }
    ...
    if (isFractional) {
        if ((flags & kUVCTypeScanFlagShowInfo))
            fprintf(stderr, "INFO: Floating-point value detected at '%s'\n", cString);
        if (theMinimum && theMaximum) {
            ...
            if (sscanf(cString, "%g%n", &fractionalValue, &n) == 1) {
                ...
                if (fractionalValue < 0.0f)
                    fractionalValue = 0.0f;
                else if (fractionalValue > 1.0f)
                    fractionalValue = 1.0f;
                ...
                *nChar = n;
                ...
                if ((flags & kUVCTypeScanFlagShowWarnings))
                    fprintf(stderr, "WARNING: Unable to scan floating-point value at '%s'\n", cString);
            }
        } else {
            if ((flags & kUVCTypeScanFlagShowWarnings))
                fprintf(stderr, "WARNING: No value range provided by control, floating-point input not available\n");
        }
    } else {
        //
        // Integer values:
        ...
        if ((flags & kUVCTypeScanFlagShowInfo))
            fprintf(stderr, "INFO: Defaulting to integer value at '%s'\n", cString);
        switch (theType) {
            case kUVCTypeComponentTypeSInt8: {
                SInt8 *min = (SInt8 *) theMinimum;
                SInt8 *max = (SInt8 *) theMaximum;
                ...
                if (sscanf(cString, "%hi%n", &val, &n) == 1) {
                    ...
                    *((SInt8 *) theValue) = val;
                    ...
                    if ((flags & kUVCTypeScanFlagShowWarnings))
                        fprintf(stderr, "WARNING: Unable to scan integer value at '%s'\n", cString);
                }
                break;
            }
            ...
            case kUVCTypeComponentTypeBitmap8: {
                ...
                if (sscanf(cString, "%hhu%n", &val, &n) == 1) {
                    ...
                    *((UInt8 *) theValue) = val;
                    ...
                    if ((flags & kUVCTypeScanFlagShowWarnings))
                        fprintf(stderr, "WARNING: Unable to scan bitmap value at '%s'\n", cString);
                }
                break;
            }
...
+ (UVCType *)uvcTypeWithCString:(const char *)typeDescription
{
    ...
    while (*typeDescription) {
        ...
        while (isspace(*typeDescription))
            typeDescription++;
        if (!*typeDescription) {
            fprintf(stderr, "WARNING: Early end to type string at %ld in: %s", (typeDescription - originalStr),
                    originalStr);
            return nil;
        }
...
+ (UVCType *)uvcTypeWithFieldNamesAndTypes:(NSString *)name, ...
{
    ...
    if (name) {
        ...
        while ((nextType = va_arg(vargs, UVCTypeComponentType))) {
            if (nextType >= kUVCTypeComponentTypeMax)
                break;
            fieldCount++;

            if ((nextName = va_arg(vargs, NSString *)) == nil)
                break;
        }
        va_end(vargs);
            ...
            if (newType && (newType = [newType init])) {
                ...
                if ((newType->_fields = object_getIndexedIvars(newType)) > 0) {
                    ...
                    while (i < fieldCount) {
                        ...
                        if (UVCTypeComponentByteSize(FIELD_PTR->fieldType) != 1)
                            needsNoByteSwap = NO;
                        i++;
                        FIELD_PTR++;
...
+ (UVCType *)uvcTypeWithFieldCount:(NSUInteger)count names:(NSString **)names types:(UVCTypeComponentType *)types
{
    ...
    while (i < count) {
        if (!names[i] || (types[i] >= kUVCTypeComponentTypeMax))
            break;
        i++;
    }
    if (i == count) {
...
{
    NSMutableString *asString = [[NSMutableString alloc]
        initWithFormat:@"UVCType@%p { field-count: %lu; byte-size: %lu; needs-byte-swap: %s; fields: {", self,
                       (unsigned long) _fieldCount, [self byteSize], _needsNoByteSwap ? "no" : "yes"];
    ...
    if ([otherObject isKindOfClass:[self class]]) {
        ...
        if (([self fieldCount] == [otherType fieldCount]) && ([self byteSize] == [otherType byteSize])) {
            ...
            while (MY_FIELD_PTR < MY_FIELD_MAX) {
                if (MY_FIELD_PTR->fieldType != THEIR_FIELD_PTR->fieldType)
                    return NO;
                MY_FIELD_PTR++;
                THEIR_FIELD_PTR++;
            }
...
- (NSUInteger)indexOfFieldWithName:(NSString *)fieldName
{
    ...
    while (FIELD_PTR < FIELD_MAX) {
        if ([FIELD_PTR->fieldName caseInsensitiveCompare:fieldName] == NSOrderedSame)
            return fieldIdx;
        FIELD_PTR++;
        fieldIdx++;
    }
...
{
    return [self scanCString:cString intoBuffer:buffer flags:flags minimum:nil maximum:nil stepSize:nil
                defaultValue:nil];
}
...
{
    return [self scanCString:cString intoBuffer:buffer flags:flags minimum:minimum maximum:maximum stepSize:nil
                defaultValue:nil];
}
       ...
       defaultValue:(void *)defaultValue
{
    ...
    if (*cString != '{') {
        ...
        if (strncasecmp(cString, "default", 7) == 0) {
            if (defaultValue) {
                if ((flags & kUVCTypeScanFlagShowInfo))
                    fprintf(stderr, "INFO: Copying default value provided by this control\n");
                memcpy(buffer, defaultValue, [self byteSize]);
                rc = YES;
            } else {
                if ((flags & kUVCTypeScanFlagShowWarnings))
                    fprintf(stderr, "WARNING: No default value provided by this control\n");
            }
        }
        //
        ...
        if (strncasecmp(cString, "minimum", 7) == 0) {
            if (minimum) {
                if ((flags & kUVCTypeScanFlagShowInfo))
                    fprintf(stderr, "INFO: Copying minimum value provided by this control\n");
                memcpy(buffer, minimum, [self byteSize]);
                rc = YES;
            } else {
                if ((flags & kUVCTypeScanFlagShowWarnings))
                    fprintf(stderr, "WARNING: No minimum value provided by this control\n");
            }
        }
        //
        ...
        if (strncasecmp(cString, "maximum", 7) == 0) {
            if (maximum) {
                if ((flags & kUVCTypeScanFlagShowInfo))
                    fprintf(stderr, "INFO: Copying maximum value provided by this control\n");
                memcpy(buffer, maximum, [self byteSize]);
                rc = YES;
            } else {
                if ((flags & kUVCTypeScanFlagShowWarnings))
                    fprintf(stderr, "WARNING: No maximum value provided by this control\n");
            }
        }
        if (_fieldCount == 1) {
            //
            ...
            //
            rc = __UVCTypeComponentTypeScanf(cString, FIELD_PTR->fieldType, buffer, flags, minimum, maximum, stepSize,
                                             defaultValue, &nChar);
        }
    } else {
        BOOL usesNamedValues = strchr(cString, '=') ? YES : NO;
        NSUInteger fieldIdx = 0;
        CFMutableBitVectorRef fieldsUsed = usesNamedValues ? CFBitVectorCreateMutable(kCFAllocatorDefault, _fieldCount)
                                                           : NULL;

        if ((flags & kUVCTypeScanFlagShowInfo))
            fprintf(stderr, "INFO: Using %s field assignments\n", (usesNamedValues ? "named-value" : "in-order"));
        ...
        while (*cString && (fieldIdx < _fieldCount)) {
            ...
            if ((flags & kUVCTypeScanFlagShowInfo))
                fprintf(stderr, "INFO: Processing substring '%s'\n", cString);
            ...
            if (usesNamedValues) {
                ...
                if (1) {
                    ...
                    if (fieldIdx == UVCTypeInvalidIndex) {
                        ...
                        if ((flags & kUVCTypeScanFlagShowWarnings))
                            fprintf(stderr, "WARNING: Field name '%s' not defined for this type\n", nameStr);
                        return NO;
                    }
                }
            }
            ...
            // Scan a value:
            BOOL scanned = __UVCTypeComponentTypeScanf(cString, FIELD_PTR[fieldIdx].fieldType, valuePtr, flags,
                                                       minimumPtr, maximumPtr, stepSizePtr, defaultValuePtr, &nChar);
            if (!scanned)
                break;
        ...
        if (fieldsUsed) {
            if (CFBitVectorContainsBit(fieldsUsed, CFRangeMake(0, _fieldCount), 0)) {
                ...
                if ((flags & kUVCTypeScanFlagShowWarnings))
                    fprintf(stderr, "WARNING: %ld named field%s %s not provided\n", zeroCount,
                            ((zeroCount == 1) ? "" : "s"), ((zeroCount == 1) ? "was" : "were"));
            } else {
                rc = YES;
            }
            CFRelease(fieldsUsed);
    ...
    if ((flags & kUVCTypeScanFlagShowInfo) && rc)
        fprintf(stderr, "INFO: Successfully scanned all fields for this type\n");
    return rc;
}
...
- (NSString *)stringFromBuffer:(void *)buffer
{
    ...
    while (FIELD_PTR < FIELD_MAX) {
        switch (FIELD_PTR->fieldType) {
            case kUVCTypeComponentTypeBoolean: {
                [asString appendFormat:@"%s%@=%s", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((UInt8 *) buffer) ? "true" : "false"];
                shouldIncludeComma = YES;
                buffer++;
                break;
            }
            case kUVCTypeComponentTypeSInt8: {
                [asString appendFormat:@"%s%@=%hhd", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((SInt8 *) buffer)];
                shouldIncludeComma = YES;
                buffer++;
                break;
            ...
            case kUVCTypeComponentTypeUInt8:
            case kUVCTypeComponentTypeBitmap8: {
                [asString appendFormat:@"%s%@=%hhu", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((UInt8 *) buffer)];
                shouldIncludeComma = YES;
                buffer++;
                break;
            ...
            case kUVCTypeComponentTypeSInt16: {
                [asString appendFormat:@"%s%@=%hd", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((SInt16 *) buffer)];
                shouldIncludeComma = YES;
                buffer += 2;
                break;
            ...
            case kUVCTypeComponentTypeUInt16:
            case kUVCTypeComponentTypeBitmap16: {
                [asString appendFormat:@"%s%@=%hu", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((UInt16 *) buffer)];
                shouldIncludeComma = YES;
                buffer += 2;
                break;
            ...
            case kUVCTypeComponentTypeSInt32: {
                [asString
                    appendFormat:@"%s%@=%d", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName, *((SInt32 *) buffer)];
                shouldIncludeComma = YES;
                buffer += 4;
                break;
            }
            ...
            case kUVCTypeComponentTypeBitmap32: {
                [asString
                    appendFormat:@"%s%@=%u", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName, *((UInt32 *) buffer)];
                shouldIncludeComma = YES;
                buffer += 4;
                break;
            }
            case kUVCTypeComponentTypeSInt64: {
                [asString appendFormat:@"%s%@=%lld", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((SInt64 *) buffer)];
                shouldIncludeComma = YES;
                buffer += 8;
                break;
            ...
            case kUVCTypeComponentTypeUInt64:
            case kUVCTypeComponentTypeBitmap64: {
                [asString appendFormat:@"%s%@=%llu", shouldIncludeComma ? "," : "", FIELD_PTR->fieldName,
                                       *((UInt64 *) buffer)];
                shouldIncludeComma = YES;
                buffer += 8;
                break;
...
- (id)jsonObjectFromBuffer:(void *)buffer
{
    ...
    // Helper to get NSNumber from buffer
    NSNumber * (^getNumber)(void *, UVCTypeComponentType) = ^NSNumber *(void *buf, UVCTypeComponentType type) {
        switch (type) {
            case kUVCTypeComponentTypeBoolean:
                return [NSNumber numberWithBool:(*(UInt8 *) buf ? YES : NO)];
            case kUVCTypeComponentTypeSInt8:
                return [NSNumber numberWithChar:*(SInt8 *) buf];
            case kUVCTypeComponentTypeUInt8:
            case kUVCTypeComponentTypeBitmap8:
                return [NSNumber numberWithUnsignedChar:*(UInt8 *) buf];
            case kUVCTypeComponentTypeSInt16:
                return [NSNumber numberWithShort:*(SInt16 *) buf];
            case kUVCTypeComponentTypeUInt16:
            case kUVCTypeComponentTypeBitmap16:
                return [NSNumber numberWithUnsignedShort:*(UInt16 *) buf];
            case kUVCTypeComponentTypeSInt32:
                return [NSNumber numberWithInt:*(SInt32 *) buf];
            case kUVCTypeComponentTypeUInt32:
            case kUVCTypeComponentTypeBitmap32:
                return [NSNumber numberWithUnsignedInt:*(UInt32 *) buf];
            case kUVCTypeComponentTypeSInt64:
                return [NSNumber numberWithLongLong:*(SInt64 *) buf];
            case kUVCTypeComponentTypeUInt64:
            case kUVCTypeComponentTypeBitmap64:
                return [NSNumber numberWithUnsignedLongLong:*(UInt64 *) buf];
            default:
    ...
    if (_fieldCount == 1) {
        id val = getNumber(buffer, FIELD_PTR->fieldType);
        if (FIELD_PTR->fieldName && [FIELD_PTR->fieldName length] > 0) {
            return [NSDictionary dictionaryWithObject:val forKey:FIELD_PTR->fieldName];
        }
        return val;
    }

    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithCapacity:_fieldCount];
    while (FIELD_PTR < FIELD_MAX) {
        [dict setObject:getNumber(buffer, FIELD_PTR->fieldType)
                 forKey:(FIELD_PTR->fieldName ? FIELD_PTR->fieldName : @"value")];
        buffer += UVCTypeComponentByteSize(FIELD_PTR->fieldType);
        FIELD_PTR++;
    }
    return dict;
}
...
- (NSDictionary *)typeSummaryDictionary
{
    ...
    while (FIELD_PTR < FIELD_MAX) {
        const char *typeStr = __UVCTypeComponentVerboseTypeString(FIELD_PTR->fieldType);
        [dict setObject:(typeStr ? [NSString stringWithUTF8String:typeStr] : @"unknown")
                 forKey:(FIELD_PTR->fieldName ? FIELD_PTR->fieldName : @"value")];
        FIELD_PTR++;
    }
    return dict;
