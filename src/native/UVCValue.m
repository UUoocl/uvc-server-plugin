...
{
    NSMutableString *asString =
        [[NSMutableString alloc] initWithFormat:@"UVCValue@%p { type: %@; bytes: [", self, _valueType];
...
- (void *)pointerToFieldAtIndex:(NSUInteger)index
{
    ...
    if (offset != kUVCTypeComponentTypeInvalid)
        return (_valuePtr + offset);
    return NULL;
}
...
{
    return [_valueType scanCString:cString intoBuffer:_valuePtr flags:flags];
}
...
{
    return [_valueType scanCString:cString intoBuffer:_valuePtr flags:flags
                           minimum:(minimum ? [minimum valuePtr] : NULL) maximum:(maximum ? [maximum valuePtr] : NULL)];
}
...
{
    return
        [_valueType scanCString:cString intoBuffer:_valuePtr flags:flags
                        minimum:(minimum ? [minimum valuePtr] : NULL) maximum
                               :(maximum ? [maximum valuePtr] : NULL) stepSize:(stepSize ? [stepSize valuePtr] : NULL)];
}
       ...
       defaultValue:(UVCValue *)defaultValue
{
    return [_valueType scanCString:cString intoBuffer:_valuePtr flags:flags
                           minimum:(minimum ? [minimum valuePtr] : NULL) maximum
                                  :(maximum ? [maximum valuePtr] : NULL) stepSize
                                  :(stepSize ? [stepSize valuePtr] : NULL) defaultValue
                                  :(defaultValue ? [defaultValue valuePtr] : NULL)];
}
...
- (id)jsonObject
{
    return [_valueType jsonObjectFromBuffer:_valuePtr];
}
