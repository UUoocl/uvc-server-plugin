...
void UvcManager::RefreshDevices() {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!found) {
            ...
            if (loggingEnabled) {
                blog(LOG_INFO, "[UVC Server] Found new device: %s (VID:%04x PID:%04x)", name.c_str(), vid, pid);
            }
        }
    ...
    for (auto &dev : devices) {
        if (dev->name == name) {
            if (dev->enabled != enabled) {
                dev->enabled = enabled;
                if (loggingEnabled) {
                    blog(LOG_INFO, "[UVC Server] SetDeviceEnabled: '%s' -> %s", name.c_str(), enabled ? "true" : "false");
                }
                if (enabled) {
                    OpenDevice(dev);
                } else {
...
void UvcManager::OpenDevice(UvcDevicePtr device) {
    if (device->nativeIndex == 9999) {
        if (loggingEnabled) blog(LOG_WARNING, "[UVC Server] Cannot open '%s': Invalid native index", device->name.c_str());
        return;
    }
    
    if (loggingEnabled) {
        blog(LOG_INFO, "[UVC Server] Attempting to open native control for '%s' (index %u)", device->name.c_str(), device->nativeIndex);
    }
    ...
    if (res == 0) {
        ...
        if (loggingEnabled) {
            blog(LOG_INFO, "[UVC Server] Native control handle opened successfully for '%s'", device->name.c_str());
        }
        BroadcastCapabilities(device->name);
    } else {
        device->isOpened = false;
        if (loggingEnabled) {
            blog(LOG_ERROR, "[UVC Server] Failed to select device '%s' for native control (result: %d)", device->name.c_str(), res);
        }
    }
}
    ...
    for (auto &dev : devices) {
        if (dev->name == deviceName && dev->isOpened) {
            ...
            if (loggingEnabled && controlsJson) {
                blog(LOG_INFO, "[UVC Server] Discovered capabilities for '%s': %s", deviceName.c_str(), controlsJson);
            }
    ...
    if (items) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            for (auto &dev : devices) {
                ...
                if ((dev->vendor_id == vid && dev->product_id == pid) || (vid == 0 && dev->name == name)) {
                    dev->enabled = enabled;
                    const char* alias = obs_data_get_string(item, "alias");
                    if (alias && *alias) dev->user_name = alias;
